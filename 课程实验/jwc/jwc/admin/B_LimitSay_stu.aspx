...
private void Page_Load(object sender,System.EventArgs e)
{
	...
	string[] TheHeadStr=new string[]{"序号","学号","姓名","行政班级"};
	...
	if(vPost!="")
	{
		...
		try{
			Response.Output.Write(ct.B_LimitSay_stu_Data(vPost,sUid));
		}catch(Exception ex){
			Response.Output.Write(ex.Message.ToString());
		}
	}
	try{
		Response.Output.Write((new purviewManage()).B_LimitSay_stu_rpt(ct.B_LimitSay_stu_rpt_Data(""),ThePer,TheAlign,TheHeadStr).ToString());
	}catch(Exception ex){
		Response.Output.Write(ex.Message.ToString());
	}
...
function SubmitUserID(value,GroupCode,GroupName,Memo){
   ...
   parent.document.all.Tck.value=1;//alert(GroupCode);
   //parent.document.all.txtXSID.value=value;
