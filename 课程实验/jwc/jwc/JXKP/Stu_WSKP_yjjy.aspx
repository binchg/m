...
string vSubmit="",vCount="",vHTML="",vITEM_DM="",vYJ="",vPJLC="",vZJ_FLAG="";
string vXN="",vXQ="";
...
if (Request.ServerVariables["REQUEST_METHOD"] == "POST" )
{
	...
	for(i=1;i<=int.Parse(vCount);i++)
	{
		...
		pSub.SaveValueWSKPXSP_WJ(vXN+vXQ,USER_ID,vJSDM,vKCDM,vITEM_DM,vYJ,vZJ_FLAG,vPJLC);
	}
	vHTML="<a  href=javascript:void(0) onclick=MM_openBrWindow(\"Stu_WSKP_yjjy.aspx?xnxq="+vXN+vXQ+"&s="+s+"&id="+USER_ID+"&kcdm="+vKCDM+"&jsdm="+vJSDM+"&kclx="+vKCLX+"&lb="+vLB+"&kclb="+vKCLB+"&pjlc="+vPJLC+"\",\"winKPT\",\"width=550,height=490,scrollbars=yes\") title=查看评分>查看</a>";
...
Response.Output.Write (PubObj.HtmlHear("提交问卷调查表","","Stu_WSKP_yjjy.aspx"));
%>
...
<%
R=pSub.GetTableWSKPXSP_WJ_PJ(vXNXQ,vKCLX,USER_ID,vJSDM,vKCDM,vLB,vKCLB,vPJLC);
if(R==0)
{
...
{
	try{Response.Output.Write (pSub.DrawRptMainWSKPXSP_WJ_PJ(vXNXQ,vLB,vKCDM,vJSDM,vKCLX,USER_ID,s,vKCLB));
	}catch(Exception){}
}
...
function go(){
	...
	form1.action='Stu_WSKP_yjjy.aspx?index='+form1.txts.value;
